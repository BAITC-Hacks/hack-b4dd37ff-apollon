...
export function runCaseChecks(dataset?: DatasetInput): CaseCheck[] {
  ...
  const directional = salesResult.quantity > regular.quantity && stockResult.quantity < regular.quantity && inboundResult.quantity < regular.quantity && lateResult.quantity === regular.quantity && growthResult.quantity > regular.quantity && categoryResult.quantity > regular.quantity;
  ...
  const checks: CaseCheck[] = [
    { id: "inputs", name: "1. Все входы меняют решение", passed: directional, details: "Синтетика. Продажи, остаток, ETA, категория и рост изменены по одному; поставка вне горизонта не уменьшает заказ.", values: { base: regular.quantity, sales: salesResult.quantity, stock: stockResult.quantity, inbound: inboundResult.quantity, lateInbound: lateResult.quantity, growth: growthResult.quantity, category: categoryResult.quantity } },
  ...
  return dataset ? [...checks, ...runDatasetChecks(dataset)] : checks;
}
