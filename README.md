...
3. Проверка расчёта одним запросом (только чтение, ничего не сохраняет) — расчёт, проверка 5 обязательных требований и топ-50 срочных позиций с объяснениями: https://apollon-production-59ea.up.railway.app/api/quick-test?limit=50
...
| `APOLLON_ACCESS_PASSWORD` | нет | Если задан, все страницы и API (кроме `/api/health`) требуют HTTP Basic-авторизацию с этим паролем (любое имя пользователя). По умолчанию не задан, чтобы жюри работало без входа. |
...
- **Нет пользователей и ролей.** Доступ к экземпляру целиком можно закрыть паролем (`APOLLON_ACCESS_PASSWORD`, `proxy.ts`), но на публичной демо-версии он выключен, чтобы жюри могло пройти сценарий. Имя утверждающего — подпись, а не проверенная личность. Не загружайте конфиденциальные данные в публичный экземпляр.
