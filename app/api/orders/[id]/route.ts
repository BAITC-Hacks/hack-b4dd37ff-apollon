...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){try{rejectCrossOrigin(request);const b=editOrderSchema.parse(await readJson(request));return Response.json({order:await editOrder((await params).id,b.expectedRevision,b.edits)});}catch(e){return apiError(e);}}
